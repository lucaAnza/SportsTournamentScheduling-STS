...
for t1 in range(0,team):
    for t2 in range(t1+1,team):
            ...
            sequential.add(at_most_one_seq(clauses , name = f't{t1}t{t2}'))
...
for t in range(0,team):
    for w in range(0,weeks):
        sequential.add(exactly_one_seq( list(vars[t,:,:,w].flatten()) , name = f't{t}w{w}' ))
...
for p in range(0,periods):
    for w in range(0,weeks):
        for t in range(team):
            ...
            clauses.append(x)
        sequential.add(exactly_k(clauses,  k = 2 ))
...
for h in range(home):
    for p in range(0,periods):
        for w in range(0,weeks):
            sequential.add(at_most_one_seq(list(vars[:,h,p,w].flatten())  , name = f"h{h}p{p}w{w}" ))
...
for t in range(team):
    for p in range(periods):
        for w in range(weeks):
            sequential.add(at_most_one_seq(list(vars[t,:,p,w].flatten()) , name = f"t{t}p{p}w{w}" ))  
...
if(optimized_version):
    h = sequential.minimize(total_imbalance)  # Set the objective function
################################ OPT ################################
...
pairwise_result = sequential.check()
