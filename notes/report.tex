...
\[
\begin{aligned}
\sum x_i \;\;\geq k &\quad\Longleftrightarrow\quad \text{``at least $k$ of the $x_i$ is true''}, \\[6pt]
\sum x_i \;\;= k   &\quad\Longleftrightarrow\quad \text{``exactly $k$ of the $x_i$ is true''}, \\[6pt]
...
\vspace{1em}
\textit{Model~2} extends the formulation of Model~1 by introducing additional constraints,
thereby increasing the overall complexity of the problem. Unlike Model~1, this
model was implemented using a single SAT encoding strategy and does not explore
alternative encodings.

The goal of Model~2 is therefore not to compare different encoding techniques,
but to evaluate the impact of a more expressive and constrained formulation on
solver performance.
...
\todo{Check if you talk about pre-computing strategy}
...
The following tables reports the results of the SAT experiments. 
...
Our decision variables are then two 3-dimensional tensors $Y_{w,p,(i,j)}$ and $H_{w,p,(i,j)}$ of boolean values, where
\begin{itemize}
